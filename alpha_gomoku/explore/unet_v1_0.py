...
class ResNetTrainer(pl.LightningModule):
    ...
    def on_fit_end(self):
        args = self.hparams
        weight_dir = utils.DATA_DIR / 'weights' / 'explore' / 'unet' / 'v1'
...
def parse_args():
    ...
    parser.add_argument('--model', default='KernelOneUNet40_2', type=str)
...
def main():
    ...
    if torch.cuda.is_available():
        ...
        gpus = 0
        accelerator = 'cpu'
    default_root_dir = utils.LOG_DIR / 'explore' / 'unet' / 'v1'
    if args.resume:
        ...
        ppl = ResNetTrainer(**args.__dict__, version='1.0',
                            description='only train one unet')
        max_epochs = ppl.hparams.epochs
        trainer = pl.Trainer(
            gpus=gpus, accelerator=accelerator, max_epochs=max_epochs,

    ...
    def __call__(self, board, actions=None):
        if board.is_over:
            if board.winner == board.player:
                ...
                return dict(), 0.0
        board_tensor = torch.LongTensor(board.vector) \
            .reshape(Board.BOARD_SIZE, Board.BOARD_SIZE).unsqueeze(0)
        ...
        outs = outs.detach().reshape(*board_tensors.size())
        out = torch.cat([
        ...
        if actions is None:
            ...
            actions = torch.LongTensor([act[0]*Board.BOARD_SIZE+act[1] for act in actions])
            legal_mask = torch.zeros(Board.BOARD_SIZE ** 2).bool()
            legal_mask.scatter_(0, actions, torch.ones_like(actions).bool())
        legal_actions = torch.arange(Board.BOARD_SIZE ** 2)[legal_mask]
        probs = F.softmax(out[legal_mask], dim=0).cpu().numpy()
        value = torch.tanh(values.mean()).cpu().detach().numpy()
        return {(act.item()//Board.BOARD_SIZE, act.item()%Board.BOARD_SIZE): p
                for act, p in zip(legal_actions, probs)}, value

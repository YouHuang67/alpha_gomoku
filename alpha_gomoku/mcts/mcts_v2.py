...
class Node(object):
    ...
    def __init__(self, prob, key, player, level,
                 cpuct=1.0, board=None, **constant):
        self._prob = prob
        self._prob_n = 1
        self.key = key
        self.player = player
    ...
    def expand(self, probs, board, node_table):
        ...
        for action, prob in probs.items():
            self.children[action] = \
                self.get_node(board, node_table, action,
                              prob, self.cpuct, **self.constant)
    ...
    @classmethod
    def get_node(cls, board, node_table, action=None,
                 prob=1.0, cpuct=1.0, **constant):
        if action is None:
            key = board.key
            player = board.player
            ...
            board = None
        return node_table.setdefault(key,
                                     cls(prob, key, player, level, cpuct,
                                         board, **constant))
